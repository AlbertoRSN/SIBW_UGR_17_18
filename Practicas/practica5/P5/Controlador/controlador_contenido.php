...
//include ('Vistas/vista_obras.php');
include ('Modelo/contenedores/contenedor_comentarios.php');
...
if($pg != '0'){ //Si la pagina no es la de inicio 
  ...
  if($nombre_pag != 'Colecciones' ){
    ...
    //Vistas
    VistaPagsInfo::muestraColecciones($colecciones);  //AQUI HABRIA QUE HABER UNA VENTANA EMERGENTE MEJOR QUE MUESTRE LAS COELLECIONES QUE HAYA Y SISE PULSA ENTONCES SALDRÁN LAS COLECCIONES CON SUS OBRAS.
  }
...
if($ob!='0' && $ob!='nada'){ //Si la pagina no es la de inicio 

  //Modelo
  $contenedor4 = new ContenedorObras();
$obra_especifica = $contenedor4->getObraByIndice($ob);

  $contenedor5 = new ContenedorComentarios();
  $comentarios = $contenedor5->getAllComentarios();
  //$obra_especifica = $contenedor4->getObraByIndice($ob,$comentarios);
  

  if( empty($obra_especifica)){
    $ob='1';
    $obra_especifica = $contenedor4->getObraByIndice($ob);
  ...
  VistaObras::muestraComentarios($comentarios);
  echo '</section>';  
}
